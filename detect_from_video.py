...
def test_full_image_network(video_path, model_path, output_path,
    ...
    print("Bitange hapa what video: "+video_path)

    # Read and write
    reader = cv2.VideoCapture(video_path)

    # Get video properties
    frame_width = reader.get(cv2.CAP_PROP_FRAME_WIDTH)
    print("Frame width:", frame_width)

    frame_height = reader.get(cv2.CAP_PROP_FRAME_HEIGHT)
    ...
    fps = reader.get(cv2.CAP_IMAGES)
    num_frames = int(reader.get(cv2.CAP_PROP_FRAME_COUNT))

    print("Bitange hapa, the total frames: "+str(num_frames))
    print("Bitange hapa, the fps: "+str(fps))
...
def detection(video_path, model_path, output_path, start_frame=100, end_frame=120, cuda=False):
    print(video_path)
    if video_path.endswith('.mp4') or video_path.endswith('.avi'):
